...
bool OverseeUI::Elements::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ...
    auto str = std::u8string(ICON_FA_CHEVRON_DOWN);

    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 30, frame_bb.Min.y + style.FramePadding.y + 12), ImColor(211, 211, 211, 211), std::string{ str.begin(), str.end() }.c_str());
...
void OverseeUI::Elements::EndCombo()
{
    ImGui::EndPopup();
}
...
bool OverseeUI::Elements::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;
...
bool OverseeUI::Elements::Combo(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items)
{
...
bool OverseeUI::Elements::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items)
{
    int items_count = 0;
