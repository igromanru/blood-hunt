...
bool OverseeUI::Elements::Tab(const char* label, std::u8string icon, int& tab, int index)
{
    ...
    it_circle->second.size = ImTricks::Animations::FastFloatLerp(id, index == tab, 54, 80, 1.f);
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(127, 128, 246, int(it_circle->second.hovered * 255)), 12);

    ImGui::PushClipRect(bb.Min, bb.Max, false);

    window->DrawList->AddText(NULL, 18, bb.Min + ImVec2(it_circle->second.size / 2 - ((ImGui::CalcTextSize(std::string(icon.begin(), icon.end()).c_str()).x) / 2) * 1.20,
        it_circle->second.size / 2 - 5),
        ImTricks::Animations::FastColorLerp(
            ImColor(210, 210, 210, 160),
            ImColor(127, 128, 246),
            it_circle->second.hovered), std::string(icon.begin(), icon.end()).c_str());
